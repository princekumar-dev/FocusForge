...
def get_dynamic_backend_url(request: Request) -> str:
    ...
    # In local development there is usually no proxy header, and the app is served over plain HTTP.
    if forwarded_proto:
        scheme = forwarded_proto
    elif effective_host.startswith(("localhost", "127.0.0.1")):
        scheme = "http"
    else:
        scheme = "https"
...
@router.post("/clerk/exchange", response_model=TokenExchangeResponse)
async def exchange_clerk_token(
    payload: ClerkTokenExchangeRequest,
    db: AsyncSession = Depends(get_db),
):
    """Exchange a Clerk session token for the app's JWT."""
    claims = decode_clerk_token(payload.clerk_token)
    ...
    auth_service = AuthService(db)
    email = payload.email or claims.get("email_address") or claims.get("email") or ""
    name = payload.name or claims.get("name") or claims.get("username")
    user = await auth_service.get_or_create_user(platform_sub=user_id, email=email, name=name)
    app_token, _expires_at, _claims = await auth_service.issue_app_token(user=user)
