...
def decode_clerk_token(token: str) -> Dict[str, Any]:
    """Decode a Clerk session token using the configured public key."""
    public_key = getattr(settings, "clerk_jwks_public_key", "")
    if not public_key:
        raise AccessTokenError("Clerk public key is not configured")
    ...
    try:
        ...
        raise AccessTokenError("Clerk token has expired") from exc
    except JWTError as exc:
        logger.warning("Clerk token validation failed: %s", type(exc).__name__)
        raise AccessTokenError("Invalid Clerk token") from exc
